...
///
/// Los montos NO se ocultan al Vendedor acá a propósito: es lo que ese cliente
...
public class ClienteDealService
{
    ...
    private static void Exigir()
    {
        if (!SesionActual.TienePermiso(Permisos.Clientes))
            throw new UnauthorizedAccessException("No tenés permiso para ver la ficha de clientes.");
    }
